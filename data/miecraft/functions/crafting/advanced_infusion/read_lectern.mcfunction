...
# 若射线终点是讲台就判断物品
execute at @e[tag=mie_ray_mark] if block ~ ~ ~ minecraft:lectern[has_book=true]{Book:{tag:{mie_id:2}}} align xyz positioned ~.5 ~.9 ~.5 as @e[type=item,tag=mie_item_ainf_core_locked,scores={mie_item_inf_t=0},distance=...1] at @s run function miecraft:crafting/advanced_infusion/recipes/items
# 物品判断正确后开始注魔
